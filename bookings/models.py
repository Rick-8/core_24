...
class Booking(models.Model):
    ...
    def clean(self):
        """
        Custom validation to ensure that no more than 50 bookings can be made
        for a given date.
        """
        if self.closed_for_maintenance:
            raise ValidationError(f"The gym is closed on {self.date}. No "
                                  "bookings can be made.")

        if Booking.objects.filter(date=self.date).count() >= 50:
            raise ValidationError("Booking limit reached for this date. "
    ...
    def __str__(self):
        return f"{self.user.username} - {self.date} - " \
               f"{'Closed' if self.closed_for_maintenance else 'Open'}"
